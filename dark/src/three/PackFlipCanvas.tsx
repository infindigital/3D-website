...
export default function PackFlipCanvas({
  ...
  return (
    ...
    >
      {/* Ambient down, key up: the pack has to stay printed artwork, but
          on a dark page a high ambient lifts the whole frame and the pack
          stops looking lit and starts looking pasted on. */}
      <ambientLight intensity={0.55} />
      <directionalLight position={[4, 6, 6]} intensity={1.75} />
      <directionalLight position={[-5, 3, 4]} intensity={0.7} color="#ffd9a8" />
      <pointLight
        position={[0, 1.8, -2]}
        intensity={6}
        distance={8}
