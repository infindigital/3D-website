...
/**
 * Fog colour: the page's own ground, so the corridor has no walls.
 *
 * It has to be the charcoal the section is painted in rather than black —
 * fog is what the far end of the corridor dissolves into, and if that
 * shade is not the page's own, the world ends in a visible disc.
 */
const GROUND = "#161110";

/**
 * How fast the corridor gives way to the ground.
 *
...
function Scene({ packs, awake, mode, onSelect }: WorldCanvasProps) {
  ...
  return (
    <>
      {/* A dark room with the packs lit inside it. The packs are printed
          artwork and still have to read as print, so the key stays strong;
          what comes down is the ambient, because ambient is the part that
          lifts the *whole* frame and would otherwise flatten the room back
          to grey. The result is a pack that looks lit rather than a page
          that looks washed. */}
      <ambientLight intensity={0.62} />
      <directionalLight position={[4, 6, 6]} intensity={1.5} />
      <directionalLight position={[-5, 3, 4]} intensity={0.72} color="#ffd9a8" />
      <TravellingLight />
...
export default function WorldCanvas({ packs, awake, mode, onSelect }: WorldCanvasProps) {
  ...
  return (
    <Canvas
      ...
      onCreated={({ scene }) => {
        scene.fog = new THREE.FogExp2(GROUND, FOG_DENSITY);
      }}
    >
